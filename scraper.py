...
def scrape_property(url, delay=1):
    """Scrapea todos los detalles de una propiedad individual"""
    try:
        res = requests.get(url, headers=HEADERS)
        soup = BeautifulSoup(res.text, "html.parser")

        # Referencia
        ref_tag = soup.select_one("#fichapropiedad-bloquecaracteristicas li span.valor")
        ...
        # Fotos (slider y principal)
        fotos = []
        slider_tags = soup.select(".fichapropiedad-listado-fotos img")
        for f in slider_tags:
...
def scrape_all_properties(listado_url, delay=1):
    """Scrapea todas las propiedades del listado con paginación"""
    propiedades = []
    page = 1
    ...
    while True:
        ...
        for a in links:
            ...
            prop_data = scrape_property(prop_url, delay)
            if prop_data:
                propiedades.append(prop_data)

        # Revisa si hay siguiente página
        next_page = soup.select_one("a.flecha2-next")
        if not next_page:
            break
...
def export_to_csv(data, filename="propiedades.csv"):
    if not data:
        return
    keys = data[0].keys()
