...
async function geocode(ciudad, zona){
 ...
 try{
  ...
  if(r){
   ...
   await guardarZonaCache({ clave, ciudad, zona, latitud: coords.lat, longitud: coords.lng })
   await delay(500)
   return coords
  }
 }catch(e){
  if(e.response?.status === 429){
   console.log("Rate limit geocoder, esperando 2s...")
   await delay(2000)
  }else{
...
app.get("/", (req,res)=>{
 res.json({ total: propiedades.length })
})
